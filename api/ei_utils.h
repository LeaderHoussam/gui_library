 ...
 * @file	ei_utils.h
 *
 * @brief	General purpose utility functions: creation of points and sizes, and arithmetic on them.
 */
...
{
	s.width		= (int)roundf((float)s.width * scale);
	s.height	= (int)roundf((float)s.height * scale);
	return s;
}
