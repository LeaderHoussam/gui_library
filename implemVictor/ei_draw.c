...
void copy_surface (ei_surface_t source, const ei_point_t origine_src, ei_surface_t destination, const ei_point_t origine_dst, ei_size_t size, bool alpha){
    ...
    for(j=0; j < size.height; j++){
        ...
        for(i=0;i < size.width;i++) {
            ...
            dst[ir] =  (dst[ir] * (255 - a_s) + r_s * a_s) / 255;
            dst[ig] =  (dst[ig] * (255 - a_s) + g_s * a_s) / 255;
            dst[ib] =  (dst[ib] * (255 - a_s) + b_s * a_s) / 255;
...
void ei_drawentry_text(ei_surface_t surface,
    ...
    for (j = 0; j < size_surf.height; j++) {
        for (i = size_surf.width - 1; i >= 0; i--) {
            //if (x_i + i >= 0 && y_i + j >= 0 && x_i + i < taille_prin.width && y_i + j < taille_prin.height &&
                //x_i + i >= pos_deb.x && x_i + i <= pos_deb.x + dim_clip.width && y_i + j >= pos_deb.y && y_i + j <= pos_deb.y + dim_clip.height) {

                int offset_surf = 4 * (j * width_surf + i);
                int offset_dest = 4 * (j * width_surface + i);

                r_s = pointeur_surf[offset_surf + ir];
                g_s = pointeur_surf[offset_surf + ig];
                b_s = pointeur_surf[offset_surf + ib];
                a_s = pointeur_surf[offset_surf + ia];

                pointeur_surface[offset_dest + ir] = (pointeur_surface[offset_dest + ir] * (255 - a_s) + r_s * a_s) / 255;
                pointeur_surface[offset_dest + ig] = (pointeur_surface[offset_dest + ig] * (255 - a_s) + g_s * a_s) / 255;
                pointeur_surface[offset_dest + ib] = (pointeur_surface[offset_dest + ib] * (255 - a_s) + b_s * a_s) / 255;
           // }
        }
    }
