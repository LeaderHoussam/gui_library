...
bool global_event_handler(ei_widget_t widget, ei_event_t* event, void* user_param)
{
	int		dir_code;
	game_t*		tm_game;
	...
	if ( (event->type == ei_ev_close) ||
		...
		dir_code		= (event->param.key_code == SDLK_UP) ? 0 :
					  (event->param.key_code == SDLK_DOWN) ? 1 :
					  (event->param.key_code == SDLK_RIGHT) ? 2 :
					  (event->param.key_code == SDLK_LEFT) ? 3 : -1;
		if (dir_code != -1) {
		...
		if (event->modifier_mask & cmd_mask) {
			if (event->param.key_code == SDLK_n) {
				// Create a new game.
				new_game(4, 4, 100, 4);
				return true;
			}
			if (event->param.key_code == SDLK_w) {
...
int main(int argc, char* argv[])
{
	...
	ei_bind(ei_ev_close, NULL, "all", global_event_handler, NULL);
